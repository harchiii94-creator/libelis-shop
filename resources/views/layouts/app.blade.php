...
<html lang="id">
    ...
    <body class="bg-[#EEEEEE] text-slate-900 antialiased overflow-x-hidden">
        ...
        <div class="fixed right-4 bottom-4 z-50 flex flex-col items-end gap-3 sm:right-5 sm:bottom-24">
            <a href="{{ route('chatbot.index') }}" class="inline-flex items-center justify-center gap-2 rounded-full bg-[#2FA884] px-4 py-3 text-sm font-semibold text-white shadow-2xl transition hover:scale-105">
            ...
            <a href="https://wa.me/{{ config('services.whatsapp.number') }}?text={{ urlencode('Halo ' . $siteName . ', saya ingin bertanya tentang produk anda') }}" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-sm font-semibold text-white shadow-2xl transition hover:scale-105">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
